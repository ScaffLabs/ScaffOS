...
const createError = (type: string, message: string) => { switch (type) { case 'ValidationError': return new ValidationError([message]); case 'NotFoundError': return new NotFoundError(message); case 'ServiceError': return new ServiceError(message); default: return new InternalServerError('An unexpected error occurred.'); } };
