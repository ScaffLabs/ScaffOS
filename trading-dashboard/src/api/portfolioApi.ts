...
export const createPosition = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Position created successfully', position: positionData });
    } catch (error) {
...
export const updatePosition = async (req: Request, res: Response) => {
    ...
    try {
        validatePosition(positionData);
        const sql = 'UPDATE positions SET quantity = ? WHERE id = ?';
        const result = await query(sql, [positionData.quantity, id]);
        if (result.affectedRows === 0) throw new NotFoundError('Position not found');
